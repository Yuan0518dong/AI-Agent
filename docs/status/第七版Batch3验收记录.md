...
- 新增 Alembic `20260717_02_ingestion_rag_pgvector`：执行 `CREATE EXTENSION IF NOT EXISTS vector`，增加资料来源/状态/定位字段，使用 `embedding_vector vector(2048)` 和 `vector_cosine_ops` HNSW 索引。旧 JSON embedding 字段保留给 SQLite 和历史兼容。
...
- `vector(2048)`、cosine HNSW、启动期 embedding 配置/维度契约及 Alembic head。
...
- 本批验证了迁移链、SQLite JSON fallback、Alembic head 和 PostgreSQL SQL 定义；未在本轮连接外部 Neon 分支执行迁移或写入生产数据。部署前仍需按 Batch 1 的直连迁移流程在隔离 Neon 分支执行 `upgrade head`，确认实际 `vector` 扩展与 HNSW 索引。
