...
A1 和 A2 已完成实现，并在授权范围内完成相同 20 × 3 的真实 DeepSeek 复跑。工具选择率由 `0.6333` 提升到 `0.75`，确认完整性由 `0.5556` 提升到 `0.6667`，失败 Run 由 22 降到 15；但仍未达到预设 `0.80/0.85` 目标，因此当前候选保持 Draft，不合并到 `main`。完整对比见 `docs/evaluation/agent-reliability-a2/A0-A2真实评测对比.md`。
...
真实复跑表明 `013` 已从 2/3 提升到 3/3，`018` 从 0/3 提升到 2/3；`014` 和 `016` 仍为 0/3，其他失败场景仅部分改善。该结果支持策略方向，但不支持“问题已经解决”的结论。
...
本次真实复跑的授权门槛为 `最多 126 次 Provider 请求、最高 $0.27`；只有在用户明确批准后才设置 `BATCH4_REAL_EVAL_APPROVED=yes`。

实际完成 93 次 Provider 请求、`147390/28533` prompt/completion Token，估算成本 `$0.028624`，保守预留成本 `$0.083698`，均未达到授权上限。
